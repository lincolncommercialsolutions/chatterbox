...
def generate_tts_audio(
    ...
    print(f"Language: {language_id}, Max tokens: {max_tokens_input}")
    ...
    try:
        ...
        print(f"Generated audio shape: {wav.shape}, dtype: {wav.dtype}")
        
        # The generate function returns numpy array directly
        if isinstance(wav, np.ndarray):
            wav_np = wav
        elif hasattr(wav, 'cpu'):  # torch tensor
            wav_np = wav.cpu().squeeze(0).numpy() if wav.dim() > 1 else wav.cpu().numpy()
        else:
            wav_np = np.asarray(wav)
        
        print(f"Final audio shape: {wav_np.shape}, dtype: {wav_np.dtype}")
        print(f"Audio sample rate: {current_model.sr}")
    ...
    with gr.Row():
        with gr.Column():
            ...
            with gr.Accordion("More options", open=False):
                ...
                max_tokens = gr.Slider(100, 1000, step=50, label="Max Audio Length (in tokens, ~20ms each)", value=400)
